...
import com.digitalojt.api.dto.ResponseMessage;
...
public class AdminAuthController {
    ...
    public ResponseEntity<?> login(@RequestBody AdminInfo adminInfo) {
        try {
            ...
            // トークンを返却する
            return ResponseEntity.ok(new ResponseMessage("Login successful", HttpStatus.OK.value(), token));

        } catch (Exception e) {
            // 認証失敗時のレスポンス
        	ResponseMessage responseMessage = new ResponseMessage("Invalid credentials", HttpStatus.UNAUTHORIZED.value(), null);
